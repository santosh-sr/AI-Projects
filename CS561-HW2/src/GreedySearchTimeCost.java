...
public class GreedySearchTimeCost extends Search{
	private PriorityQueue<EdgeCost> priorityQueue;

	public GreedySearchTimeCost(Map<String, List<Node>> graphStructure){
		super(graphStructure);
		priorityQueue = new PriorityQueue<EdgeCost>(100, new DirectTimeCostComparator());
	}

	private EdgeCost getGreedyTimeCost(Node goalNode){
	...
	public Stack<EdgeCost> getGreedySearchByTime(String startNode, String goalNode){
		Stack<EdgeCost> pathResult = new Stack<EdgeCost>();

		priorityQueue.add(new EdgeCost(null, 0, 0, new Node(startNode, 0, 0)));
		EdgeCost goalPathCostNode = getGreedyTimeCost(new Node(goalNode, 0, 0));
